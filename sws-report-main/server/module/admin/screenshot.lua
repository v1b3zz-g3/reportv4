-- server/module/admin/screenshot.lua - Enhanced version with chunked data transfer
...
---@type table<integer, table> Screenshot data chunks being assembled
local screenshotChunks = {}
local SCREENSHOT_COOLDOWN = 10000 -- 10 seconds
...
---Main screenshot handler - tries screenshot-basic method
local function captureScreenshot(targetSource, reportId, playerName, onComplete)
    ...
    if not basicSuccess then
        ...
        onComplete(false, nil, nil, basicError or "Screenshot system unavailable")
    end
end

---Receive screenshot data chunk from client
RegisterNetEvent("sws-report:screenshotChunk", function(chunkIndex, totalChunks, chunkData, reportId)
    local source = source
    
    if not screenshotChunks[source] then
        screenshotChunks[source] = {
            chunks = {},
    ...
    screenshotChunks[source].chunks[chunkIndex] = chunkData
    screenshotChunks[source].receivedCount = screenshotChunks[source].receivedCount + 1
    
    print(("^3[screenshot-chunk] Received chunk %d/%d from source %d^0"):format(
        screenshotChunks[source].receivedCount, 
        totalChunks, 
        source
    ))
    
    -- Check if all chunks received
    if screenshotChunks[source].receivedCount == totalChunks then
        print(("^2[screenshot-chunk] All chunks received, assembling...^0"))
        
        -- Assemble chunks in order
        local fullData = ""
        for i = 1, totalChunks do
            if screenshotChunks[source].chunks[i] then
                fullData = fullData .. screenshotChunks[source].chunks[i]
            else
                print(("^1[screenshot-chunk] Missing chunk %d!^0"):format(i))
                NotifyPlayer(source, "Screenshot transfer failed - missing data", "error")
                screenshotChunks[source] = nil
                setScreenshotInProgress(source, false)
                return
            end
        end
        
        local reportId = screenshotChunks[source].reportId
        screenshotChunks[source] = nil
        
        print(("^2[screenshot-chunk] Assembly complete: %d bytes^0"):format(#fullData))
        
        -- Process the complete screenshot data
        ...
        -- Upload to Discord
        uploadToDiscord(fullData, reportId, player.name, function(success, url, errorMsg)
            setScreenshotInProgress(source, false)
            ...
            if success and url then
                ...
                local errorMessage = errorMsg or "Screenshot upload failed"
...
--    Wait(3000)
--    print("^3[screenshot] Enhanced module loaded with chunked data transfer^0")
